...
export class SlideApi {
  ...
  async create(
    presentationId: number,
    data: Omit<CreateSlideRequest, 'presentationId'>
  ...
  async moveUp(
    presentationId: number,
    slideId: number
  ): Promise<ApiResponse<{ moved: boolean; newOrder: number }>> {
  ...
  async updateContent(
    ...
    return api.patch<
      ...
      { content: SlideContent }
    >(API_ENDPOINTS.slides.update(presentationId, slideId), { content });
  }
   ...
   * スライドテンプレートの取得
   */
  async getTemplates(type?: 'multiple_choice' | 'word_cloud'): Promise<
    ApiResponse<
      Array<{
        id: string;
        name: string;
  ...
  async bulkDuplicate(
    ...
    return api.post<
      ApiResponse<{ duplicated: boolean; newSlides: SlideInfo[] }>
