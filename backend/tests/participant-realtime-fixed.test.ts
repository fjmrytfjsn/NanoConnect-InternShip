/**
 * 参加者リアルタイム機能のテスト
 * JoinPresentationRealtimeUseCase の動作を検証
 */

import { JoinPresentationRealtimeUseCase, JoinPresentationRealtimeRequestDto } from '../src/application/useCases/participant/JoinPresentationRealtime';
...
describe('JoinPresentationRealtimeUseCase', () => {
  ...
  describe('プレゼンテーション参加', () => {
    it('有効なアクセスコードでプレゼンテーションに参加できる', async () => {
      ...
      presentation.start(); // プレゼンテーションをアクティブ化
    ...
    it('無効なアクセスコードでエラーが返される', async () => {
      ...
      expect(response.error).toBe('アクセスコードは6桁である必要があります'); // 実際のエラーメッセージに合わせて修正
    });
